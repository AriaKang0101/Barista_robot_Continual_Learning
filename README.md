...
4. Task A는 원본 `/FirstTarget`, `/EndTarget` 좌표를 사용합니다. 거친 grid는 연결성에만 사용하고, 먼저 유망한 grid 사이를 연속 관절각으로 세분 탐색합니다. 여기서 찾지 못하면 머신 근처의 좁은 목표영역을 놓치지 않도록 전체 관절 범위를 4배 해상도로 검사하고 안전 연결 graph까지 이어지는 자세만 경로 종점으로 채택합니다. 원본 PPO와 같이 목표 종점은 충돌 여부로 판정하며 1 cm 여유는 요구하지 않습니다.
